...
// Gmail (ai.portale3d@gmail.com) — send/read via Gmail API, better inbox
// deliverability than mail() on shared hosting (OVH shared IPs get flagged).
// Reuses the same Google OAuth client already configured for Helios7 sign-in.
// One-time setup: visit gmail_auth.php?k=MCP_SECRET to obtain GMAIL_REFRESH_TOKEN.
define('MCP_GMAIL_CLIENT_ID',     (string)($__env['GOOGLE_CLIENT_ID']     ?? ''));
define('MCP_GMAIL_CLIENT_SECRET', (string)($__env['GOOGLE_CLIENT_SECRET'] ?? ''));
define('MCP_GMAIL_REFRESH_TOKEN', (string)($__env['GMAIL_REFRESH_TOKEN']  ?? ''));
define('MCP_GMAIL_FROM',          (string)($__env['GMAIL_FROM'] ?? 'ai.portale3d@gmail.com'));

// Shared low-level Gmail helpers (gmail_access_token / gmail_api / gmail_send_mail),
// also used by 3d/faro/lib/mailer.php — kept out of the per-project files so the
// OAuth/curl logic lives in exactly one place.
$__gmailMailer = dirname(__DIR__) . '/gmail_mailer.php';
if (is_file($__gmailMailer)) require_once $__gmailMailer;
...
// ---- Gmail (OAuth2 refresh-token flow, ai.portale3d@gmail.com) -----------------
// Low-level token/API calls (gmail_access_token, gmail_api) live in the shared
// gmail_mailer.php required above. These are the thin Ykan-side wrappers.

/** Send an HTML email through Gmail. Throws McpError on failure. Returns the message id. */
function mcp_gmail_send(string $to, string $subject, string $html): string {
    if (!function_exists('gmail_send_mail')) throw new McpError('gmail_mailer.php mancante sul server.');
    [$ok, $info] = gmail_send_mail($to, $subject, $html, 'Ykan AI <' . MCP_GMAIL_FROM . '>',
        MCP_GMAIL_CLIENT_ID, MCP_GMAIL_CLIENT_SECRET, MCP_GMAIL_REFRESH_TOKEN);
    if (!$ok) throw new McpError("Gmail send failed: $info");
    return $info; // message id
}

/** List recent Gmail messages (optionally filtered by a Gmail search query), with metadata. */
function mcp_gmail_list(string $query, int $max): array {
    if (!function_exists('gmail_access_token')) throw new McpError('gmail_mailer.php mancante sul server.');
    if (MCP_GMAIL_CLIENT_ID === '' || MCP_GMAIL_CLIENT_SECRET === '' || MCP_GMAIL_REFRESH_TOKEN === '') {
        throw new McpError('Gmail non configurato: mancano GOOGLE_CLIENT_ID/SECRET o GMAIL_REFRESH_TOKEN in .env.');
    }
    $max = max(1, min(25, $max));
    try {
        $token = gmail_access_token(MCP_GMAIL_CLIENT_ID, MCP_GMAIL_CLIENT_SECRET, MCP_GMAIL_REFRESH_TOKEN);
        $url = 'https://gmail.googleapis.com/gmail/v1/users/me/messages?maxResults=' . $max
             . ($query !== '' ? '&q=' . urlencode($query) : '');
        $list = gmail_api($token, 'GET', $url);
        $out = [];
        foreach (($list['messages'] ?? []) as $m) {
            $msg = gmail_api($token, 'GET', 'https://gmail.googleapis.com/gmail/v1/users/me/messages/' . $m['id']
                . '?format=metadata&metadataHeaders=From&metadataHeaders=Subject&metadataHeaders=Date');
            $headers = array_column($msg['payload']['headers'] ?? [], 'value', 'name');
            $out[] = [
                'id'      => $m['id'],
                'from'    => $headers['From']    ?? '?',
        ...
        throw new McpError('Gmail read failed: ' . $e->getMessage());
    }
}
...
function mcp_tool_defs(): array {
    ...
    return [
        ...
        [
            'name' => 'send_gmail',
            'description' => 'Send an email via Gmail (ai.portale3d@gmail.com) instead of PHP mail(). Much better inbox deliverability — use this instead of send_digest_mail when the recipient risks landing in spam.',
            'inputSchema' => ['type' => 'object', 'properties' => [
        ...
        [
            'name' => 'read_gmail',
            'description' => 'List recent emails from the ai.portale3d@gmail.com inbox (sender, subject, date, snippet). Optional Gmail search query, e.g. "is:unread" or "from:someone@example.com".',
            'inputSchema' => ['type' => 'object', 'properties' => [
                'query' => ['type' => 'string', 'description' => 'Gmail search query (optional).'],
                'max'   => ['type' => 'integer', 'description' => 'Max messages to return (default 10, max 25).'],
            ]],
        ],
        // -------- Phase 0: Google Tasks sync --------
        [
            'name' => 'sync_google_tasks',
            'description' => 'Sync Ykan tasks with Google Tasks (one Google Tasklist per project). Completion status is bidirectional; title/description/priority/due date only flow Ykan -> Google. Also runs automatically every cron tick.',
            'inputSchema' => ['type' => 'object', 'properties' => (object)[]],
        ],
    ];
}
...
function mcp_run_tool(string $name, array $a): string {
    ...
    switch ($name) {
        ...
        case 'move_task': {
            ...
            if (($data['cards'][$i]['column_id'] ?? '') !== $target['id']) $data['cards'][$i]['moved_at'] = date('Y-m-d H:i:s');
        ...
        case 'send_digest_mail': {
            ...
            // Prefer Gmail (better deliverability than mail() on OVH shared hosting);
            // fall back to mail() if Gmail isn't configured or the send fails, so the
            // nightly routine never silently loses its digest.
            if (function_exists('gmail_send_mail') && MCP_GMAIL_REFRESH_TOKEN !== '') {
                [$ok, $info] = gmail_send_mail(MCP_MAIL_TO, $subject, $html, 'Ykan AI <' . MCP_GMAIL_FROM . '>',
                    MCP_GMAIL_CLIENT_ID, MCP_GMAIL_CLIENT_SECRET, MCP_GMAIL_REFRESH_TOKEN);
                if ($ok) {
                    mcp_audit('send_digest_mail', ['to' => MCP_MAIL_TO, 'subject' => $subject, 'via' => 'gmail', 'id' => $info]);
                    return "Mail sent to " . MCP_MAIL_TO . " via Gmail — subject: $subject (id: $info)";
                }
                $gmailError = $info;
            } else {
            ...
            mcp_audit('send_digest_mail', ['to' => MCP_MAIL_TO, 'subject' => $subject, 'via' => 'mail()', 'ok' => $ok, 'gmail_error' => $gmailError]);
            if (!$ok) throw new McpError("Gmail failed ($gmailError) and mail() fallback failed too: $info");
            return "Mail sent to " . MCP_MAIL_TO . " via mail() fallback (Gmail: $gmailError) — subject: $subject";
        }
        ...
        case 'send_gmail': {
            ...
            $subject = trim((string)($a['subject'] ?? ''));
            $html = (string)($a['html'] ?? '');
            if ($to === '' || $subject === '' || $html === '') throw new McpError('to, subject and html are required.');
            if (!filter_var($to, FILTER_VALIDATE_EMAIL)) throw new McpError("'$to' non è un indirizzo email valido.");
            $id = mcp_gmail_send($to, $subject, $html);
            mcp_audit('send_gmail', ['to' => $to, 'subject' => $subject, 'id' => $id]);
            return "Email inviata a $to via Gmail (id: $id).";
        }

        case 'read_gmail': {
            $query = trim((string)($a['query'] ?? ''));
            $max = (int)($a['max'] ?? 10);
            $msgs = mcp_gmail_list($query, $max);
            mcp_audit('read_gmail', ['query' => $query, 'count' => count($msgs)]);
            if (!$msgs) return 'Nessuna email trovata.';
            $out = [];
            foreach ($msgs as $m) {
                $out[] = "- [{$m['date']}] {$m['from']} — {$m['subject']}\n  {$m['snippet']}";
            }
            return implode("\n", $out);
        }

        case 'sync_google_tasks': {
            $syncFile = __DIR__ . '/tasks_sync.php';
            if (!is_file($syncFile)) throw new McpError('tasks_sync.php mancante sul server.');
            require_once $syncFile;
            if (!function_exists('ykan_tasks_sync')) throw new McpError('ykan_tasks_sync non disponibile.');
            $result = ykan_tasks_sync();
            mcp_audit('sync_google_tasks', ['ok' => $result['ok'] ?? false] + ($result['stats'] ?? []));
            if (empty($result['ok'])) throw new McpError($result['error'] ?? 'Sync fallita.');
